...
and external links. Use the object form to add an optional primary tab row above
...
Tabs are omitted when `tabs` is not set. A tab stays active for pages nested
below its link; `/` matches only the home page. Every internal route named in
